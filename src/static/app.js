      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length > 0
          ? `<div class="participants-list">${participants.map((participant) => `
              <div class="participant-pill" data-email="${participant}">
        ...
        activityCard.querySelectorAll(".participant-delete").forEach((button) => {
          button.addEventListener("click", async () => {
            ...
            const response = await fetch(`/activities/${encodeURIComponent(name)}/participants/${encodeURIComponent(participantEmail)}`, {
              method: "DELETE",
            });
